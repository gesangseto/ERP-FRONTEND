...
export const getStock = async (property = {}, useAlert = true) => {
  ...
  return new Promise((resolve) => {
    $axios
      .get(`/api/pos/transaction/stock?${query_string}`)
      .then((result) => {
...
export const getStockByUser = async (property = {}, useAlert = true) => {
  if (property.hasOwnProperty("search")) {
    if (Array.isArray(property.search)) {
      property.search = JSON.stringify(property.search);
    }
  ...
  return new Promise((resolve) => {
    $axios
      .get(`/api/pos/transaction/stock/by-branch?${query_string}`)
      .then((result) => {
        let _res = result.data;
        if (_res.error && useAlert) {
          toast.error(`${_res.message}`);
